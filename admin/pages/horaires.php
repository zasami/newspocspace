...
.horaire-preview-badge {
  ...
  border-radius: 6px; font-weight: 700; font-size: 1.1rem; display: inline-block;
}
.horaire-form-flex { display: flex; flex-direction: column; overflow: hidden; flex: 1 1 auto; }
.horaire-modal-body-scroll { overflow-y: auto; flex: 1 1 auto; }
.horaire-section-title { font-size: 0.72rem; letter-spacing: 1px; }
...
.horaire-color-swatch { display: inline-block; width: 24px; height: 24px; border-radius: 4px; }
</style>

<div class="d-flex justify-content-between align-items-center mb-3">
  <p class="text-muted mb-0">Codes horaires utilisés dans les plannings (A1, D3, S4, etc.)</p>
  <button type="button" class="btn btn-primary btn-sm" id="btnNewHoraire">
...
<div class="card">
  <div class="table-responsive">
    <table class="table table-hover mb-0">
      <thead>
        <tr>
          <th>Code</th><th>Nom</th><th>Début</th><th>Fin</th>
          <th>Pauses payées</th><th>Pauses non payées</th>
          <th>Durée effective</th><th>Couleur</th><th>Actif</th><th>Actions</th>
        </tr>
      </thead>
      <tbody id="horairesTableBody">
        <?php if (empty($horaires)): ?>
        <tr><td colspan="10" class="text-center py-4 text-muted">Aucun horaire configuré</td></tr>
        <?php else: ?>
        <?php foreach ($horaires as $h):
            $active = (int)($h['is_active'] ?? 1) !== 0;
            $couleur = h($h['couleur'] ?: '#ccc');
        ?>
        <tr class="clickable-row <?= !$active ? 'text-muted' : '' ?>"
            data-row-horaire="<?= h($h['id']) ?>"
            data-horaire='<?= json_encode($h, JSON_HEX_APOS | JSON_HEX_TAG) ?>'>
          <td>
            <span class="horaire-code-badge <?= $active ? '' : 'horaire-code-badge--inactive' ?>"
                  <?= $active ? 'style="background:' . $couleur . '"' : '' ?>>
              <?= h($h['code']) ?>
            </span>
          </td>
          <td><?= h($h['nom']) ?></td>
          <td><?= h(substr($h['heure_debut'] ?? '', 0, 5)) ?></td>
          <td><?= h(substr($h['heure_fin'] ?? '', 0, 5)) ?></td>
          <td><?= (int)$h['pauses_payees'] ?></td>
          <td><?= (int)$h['pauses_non_payees'] ?></td>
          <td><strong><?= h($h['duree_effective']) ?>h</strong></td>
          <td><span class="horaire-color-swatch" style="background:<?= $couleur ?>"></span></td>
          <td>
            <div class="form-check form-switch mb-0">
              <input class="form-check-input" type="checkbox" <?= $active ? 'checked' : '' ?>
                     data-toggle-horaire="<?= h($h['id']) ?>">
            </div>
          </td>
          <td>
            <button type="button" class="btn btn-sm btn-horaire-edit"
                    data-edit-horaire="<?= h($h['id']) ?>">
        ...
        <div class="modal-body horaire-modal-body-scroll">
          <div class="mb-4">
            ...
            <div class="row g-3">
              <div class="col-md-3">
                ...
                <input type="text" class="form-control form-control-lg text-center fw-bold horaire-code-input"
                       id="horaireCode" required maxlength="10" placeholder="A1">
              </div>
              <div class="col-md-6">
                <label class="form-label fw-semibold">Nom <span class="text-danger">*</span></label>
                <input type="text" class="form-control" id="horaireNom" required
                       placeholder="Ex: Matin court, Après-midi, Soir...">
              </div>
              <div class="col-md-3">
                <label class="form-label fw-semibold">Couleur</label>
                <div class="d-flex align-items-center gap-2">
                  <input type="color" class="form-control form-control-color horaire-color-picker"
                         id="horaireCouleur" value="#1a1a1a">
          ...
          <div class="mb-4">
            ...
            <div class="row g-3">
              ...
              <div class="col-md-4">
                ...
                <div class="form-control bg-light fw-bold text-success horaire-readonly horaire-duree-eff"
                     id="horaireDureeEff">—</div>
              </div>
            </div>
          </div>
          <hr>
          <div>
            ...
            <div class="form-check form-switch">
              ...
              <label class="form-check-label fw-semibold ms-2" for="horaireActif">
                Actif — disponible pour la planification
              </label>
            </div>
          </div>
        </div>
...
(function() {
    ...
    tbody.addEventListener('change', async (e) => {
        const sw = e.target.closest('[data-toggle-horaire]');
        if (!sw) return;
        const res = await adminApiPost('admin_toggle_horaire', {
            id: sw.dataset.toggleHoraire, is_active: sw.checked ? 1 : 0
        });
        if (res.success) {
    ...
    // ── Clic sur ligne ou bouton éditer ──────────────────────────────────────
    tbody.addEventListener('click', (e) => {
        if (e.target.closest('.form-check-input')) return;
        const row = e.target.closest('[data-row-horaire]');
        if (!row) return;
        editHoraire(JSON.parse(row.dataset.horaire));
    });
    ...
    document.getElementById('horaireDeleteBtn').addEventListener('click', async () => {
        ...
        if (!await adminConfirm({
            title: "Désactiver l'horaire",
            text: 'Cet horaire ne sera plus utilisable dans la génération de planning.',
            icon: 'bi-clock-history', type: 'warning', okText: 'Désactiver'
        })) return;
    ...
    function calcDuration() {
        ...
        bruteEl.textContent = `${Math.floor(mins / 60)}h${mins % 60 > 0 ? String(mins % 60).padStart(2, '0') : ''}`;
        effEl.textContent = `${(( mins - pnp * 30) / 60).toFixed(2)}h`;
    }
    ...
    function editHoraire(h) {
        ...
        document.getElementById('horaireModalSubtitle').textContent =
            h.nom + ' · ' + (h.heure_debut || '').substring(0, 5) + ' – ' + (h.heure_fin || '').substring(0, 5);
